...
class SistemaRAG:
    ...
    def _lexical_search(self, consulta: str, k: int) -> list[dict[str, Any]]:
        ...
        for item in self._fallback_store:
            doc = item["texto"]
            meta = item["metadata"]
            text_terms = set(re.findall(r"[a-zA-Z0-9]+", doc.lower()))
            author_terms = set(re.findall(r"[a-zA-Z0-9]+", (meta.get("autor", "") or "").lower()))
            overlap_query = query_terms.intersection(text_terms)
            overlap_author = query_terms.intersection(author_terms)
            score = float(len(overlap_query)) + float(len(overlap_author)) * 2
            if score <= 0:
                continue
            items.append({
                "texto_indexado": doc,
        ...
        items.sort(key=lambda i: i.get("lexical_score", 0.0), reverse=True)
    ...
    def buscar_relevantes(self, query: str, k: int = 3) -> list[dict[str, Any]]:
        ...
        lexical: list[dict[str, Any]] = self._lexical_search(consulta, k * 2)
        ...
        merged.sort(key=lambda i: i.get("lexical_score", i.get("distancia", 1)))

        # boost items that appear in both searches
        lexical_ids = {i["metadata"]["id"] for i in lexical}
        semantic_ids = {i["metadata"]["id"] for i in semantic}
        both = lexical_ids & semantic_ids
        for item in merged:
            if item["metadata"]["id"] in both:
                item["distancia"] = max(0.0, item.get("distancia", 1) - 0.3)

        merged.sort(key=lambda i: i.get("distancia", 1))
