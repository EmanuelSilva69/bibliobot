...
def post_chat(base_url: str, payload: dict[str, Any]) -> tuple[int, dict[str, Any]]:
    # Increase timeout to allow slower LLM responses (LMStudio may need more time)
...
def test_clarificacao(base_url: str) -> TestResult:
    ...
    try:
        ...
        success = (
            status == 200
            and data.get("intent") == "busca"
            and len(answer) > 20
        )
        return TestResult(name, success, f"status={status}, intent={data.get('intent')}, answer_len={len(answer)}")
    except Exception as exc:
        return TestResult(name, False, f"erro: {exc}")
...
def main() -> int:
    ...
    # Default base URL can be overridden by the BASE_URL environment variable
    import os

    parser.add_argument(
        "--base-url",
        default=os.getenv("BASE_URL", "http://localhost:18000"),
